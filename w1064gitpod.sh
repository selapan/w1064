...
qemu-system-x86_64 -hda w10x64.img -m 64G -smp cpu=16,cores=8,threads=2,sockets=1,maxcpus=16 -net user,hostfwd=tcp::3388-:3389 -net nic -object rng-random,id=rng0,filename=/dev/urandom -device virtio-rng-pci,rng=rng0 -vga vmware -nographic &>/dev/null &
clear
echo RDP Address:
